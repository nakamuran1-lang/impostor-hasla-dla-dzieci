...
function render(){
 ...
 for(let i=0;i<pc.value;i++){
  players.innerHTML+=`<input id=p${i} value="${localStorage['p'+i]||''}" placeholder="Imię ${i+1}"><br>`;
 }
}
pc.onchange=render;
render();
...
function showPass(){
 ...
 passText.innerText='📱 Przekaż telefon: '+playersArr[current];
 slider.value=0;
}
...
slider.addEventListener('input',function(){
 if(+this.value>=95){
  ...
  roleContent.innerHTML=r.imp?
  `<div class="impostor">
    <h1 style="font-size:70px">🚨</h1>
    <h1>JESTEŚ IMPOSTOREM</h1>
    <h2>📚 ${r.cat}</h2>
   </div>`
  :
  `<div class="normal">
    <h1 style="font-size:70px">🔑</h1>
    <h1 style="font-size:50px">${r.word.toUpperCase()}</h1>
    <h2>📚 ${r.cat}</h2>
   </div>`;
 }
});
...
function nextP(){
 current++;
 if(current>=playersArr.length){
  roleScreen.classList.add('hidden');
  hiddenRole.classList.add('hidden');
  readyScreen.classList.remove('hidden');
 } else showPass();
}
nextBtn.onclick=nextP;
nextBtn2.onclick=nextP;
...
startRoundBtn.onclick=()=>{
 ...
 let d=+roundTime.value;
 timer.innerText=String(Math.floor(d/60)).padStart(2,'0')+':00';
};
...
showResultBtn.onclick=()=>{
 ...
 .map((r,i)=>{
   ...
   return `<tr><td>${medal} ${r[0]}</td><td>${r[1]}</td></tr>`;
 }).join('');
